<template>
  <section style="max-width:1100px; margin:2rem auto;">
    ...
    <!-- Filtros -->
    <form @submit.prevent="applyFilters" style="display:flex; gap:.5rem; flex-wrap:wrap; align-items:end; margin:1rem 0;">
    ...
    <table v-else style="width:100%; border-collapse:collapse; margin-top:1rem;">
      ...
      <tbody>
        <tr v-for="t in tickets" :key="t.id">
          ...
          <td style="padding:.5rem; border-bottom:1px solid #eee;">{{ t.estudiante?.nombre }} {{ t.estudiante?.apellido }}</td>
          <td style="padding:.5rem; border-bottom:1px solid #eee;">{{ t.estudiante?.cedula }}</td>
          <td style="padding:.5rem; border-bottom:1px solid #eee;">{{ t.estudiante?.facultad || '-' }}</td>
          ...
          <td style="padding:.5rem; border-bottom:1px solid #eee;">{{ new Date(t.createdAt).toLocaleString() }}</td>
          <td style="padding:.5rem; border-bottom:1px solid #eee;">
            <RouterLink :to="`/tickets/${t.id}`"><button>Ver</button></RouterLink>
          </td>
        </tr>
      </tbody>
    </table>
...
async function load() {
  ...
  try {
    const { tickets: rows } = await manageApi.adminTickets({
      q: q.value, estado: estado.value, date_from: date_from.value, date_to: date_to.value
    })
    tickets.value = rows || []
  } catch (e) {
